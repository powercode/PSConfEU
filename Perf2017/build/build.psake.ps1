...
Task InlineModules -requiredVariables ModuleOutDir, ModuleName, NonSrcDirs {
    $psm1Path = "$ModuleOutDir/$ModuleName.psm1"
    $psm1Content = Get-Content $psm1Path
    $using = @()
    Select-String -LiteralPath $psm1Path -Pattern 'using module \..(.+)' | Foreach-Object {
        $line = $_.LineNumber - 1
...
Task Build -depends Init, Clean, BeforeBuild, StageFiles, BuildDotNet, Analyze, Sign, AfterBuild {
}
...
Task BuildDotNet -requiredVariables CSSrcDir, ModuleOutDir, ModuleName {
    ...
    $csFiles = Get-ChildItem $CSSrcDir -Filter *.cs | ForEach-Object Fullname
    $dlls = $csFiles.Foreach( {[io.path]::ChangeExtension($_, '.dll')})
